...
int WriteContainerToFile (const std::string& file_path, std::vector<Event> & container){
  ...
  for (auto corner : container) {
    file << std::setprecision(17) << corner.t << " " << corner.x << " " << corner.y << " " << corner.p << std::endl;
  }
  file.close();
}
...
int main(int argc, char *argv[]) {
  ...
  if (argc==3) {
      ...
      std::cout << "Writing corners to file..." << std::flush;
